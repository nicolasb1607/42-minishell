...
/*   Updated: 2022/03/23 15:24:444 by ngobert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_export(t_tlist *tlst, t_dlist **dup_env)
{
		...
		&& ft_containchar('=', tlst->token->content))
	{
		if (spread_on_one_tok(tlst, currenv, varexport, dup_env) == 1)
			...
			return (2);
		else if (check_equal(tlst->token->content, tlst) == 0  && is_existing(varexport, dup_env) == 1)
		{
		...
		else if (check_equal(tlst->token->content, tlst) == 0  && is_existing(varexport, dup_env) == 0)
		{
			new = ft_dlstnew(ft_strdup(tlst->token->content));
			ft_dlstadd_back(dup_env, new);
			return (1);
